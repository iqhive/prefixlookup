...
// subnets - is checked against them. if the tree disagrees with a scan of the
// input, the tree is wrong
...
//
// supernets must also be strictly most-specific-first (equal lengths are
// fine; a longer prefix after a shorter one is a fail). subnet walks get
...
// we build with nil entries and probe a handful of addrs plus an exact /8 -
// both Lookup and Exact must miss
...
// fringe and then split when the next longer one arrives. that's the insert
// case we most want a regression on
...
//
// 200 random insert/delete steps, every 17th we probe. asEntries rebuilds the
// oracle list from `live` so the linear scan sees the same set the table has
...
// pointer, new value vector. /16 must still win for 10.0.0.1
